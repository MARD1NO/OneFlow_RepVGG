...
def load_imagenet_for_training(args):
    ...
    #     rsz = flow.image.Resize(image, interpolation_type="bilinear", target_size=[args.image_size, args.image_size])
    #     image = rsz[0]
    ...
    image = flow.data.OFRecordImageDecoderRandomCrop(
        ofrecord, "encoded", color_space=color_space, random_area=[0.08, 1.0], 
        random_aspect_ratio=[3./4., 4./3.]
    )
    rsz = flow.image.Resize(
        image, interpolation_type="bilinear", target_size=[args.image_size, args.image_size])
...
def load_imagenet_for_validation(args):
    ...
    rsz = flow.image.Resize(
        ...
        keep_aspect_ratio=True, interpolation_type="bilinear", 
